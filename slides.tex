...
\setbeamertemplate{frame footer}{{\textbf{Robotics at MVA:}}\insertshorttitle}
...
\begin{frame}{PPO metrics}
    \begin{itemize}
        \item \mintinline{python}{approx_kl}: approximate KL divergence between the old policy and the new one. Should go down over training but won't reach zero.
        \item \mintinline{python}{clip_fraction}: mean fraction of surrogate loss that was clipped (above the clip-range threshold).
        \item \mintinline{python}{clip_range}: current value of the clipping factor for the surrogate loss
        \item \mintinline{python}{explained_variance}: indicates whether the value function is a good predictor for returns (EV $> 1$) or if it’s just worse than predicting nothing (EV $\leq 0$).
        \item \mintinline{python}{learning_rate}: Current learning rate value.
        \item \mintinline{python}{n_updates}: Number of gradient updates applied so far.
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{PPO loss function}
    ...
    \begin{itemize}
        ...
        \item \mintinline{python}{entropy_loss}: mean value of the entropy loss (negative of the average policy entropy). It should increase to zero over training as the policy becomes more deterministic.
            \begin{itemize}
                \item Minimum when all actions are equally likely.
                \item Maxes out to zero when the same action is selected every time.
                \item Ideally gets "close" to zero but not zero (keep a sane amount of exploration).
            \end{itemize}
...
\begin{frame}{Optimizer parameters: steps, epochs, mini-batching}
    ...
    \begin{itemize}
        \item \mintinline{python}{learning_rate}: step size initialization parameter.
        \item \mintinline{python}{n_steps}: number of environment steps to collect per rollout buffer.
        \item \mintinline{python}{n_epochs}: number of times we use a rollout buffer while optimizing the surrogate loss.
...
\begin{frame}{PPO parameters: others}
    \begin{itemize}
        \item \mintinline{python}{clip_range}: clipping factor in policy loss.
        \item \mintinline{python}{clip_range_vf}: ...
        \item \mintinline{python}{ent_coef}: ...
        \item \mintinline{python}{gae_lambda}: ...
        \item \mintinline{python}{max_grad_norm}: ...
        ...
        \item \mintinline{python}{sde_sample_freq}: ...
        \item \mintinline{python}{target_kl}: ...
        \item \mintinline{python}{use_sde}: ...
        \item \mintinline{python}{vf_coef}: ...
    \end{itemize}
\end{frame}
...
\begin{frame}{Example: Upkie ground velocity}
    \begin{itemize}
        \item $\theta$: pitch from torso to inertial frame.
        \item $p$: ground position, estimated from wheel odometry.
        \item Observation: $o = (\theta, p, \dot{\theta}, \dot{p})$.
        \item Action: $v$ commanded ground velocity.
    \end{itemize}
\end{frame}
...
\begin{frame}{Domain randomization}
    ...
    \begin{itemize}
        \item Robot geometry: limb lengths, wheel diameters, ...
        \item Inertias: masses, mass distribution
        \item Initial state: $s_0 \sim \rho_0(\cdot)$
        \item Delays, actuator response models
    \end{itemize}
\end{frame}

\begin{frame}{Curriculum}
    \begin{itemize}
        \item Domain randomization can vary over time (not clearly useful)
        \item Learning rate schedule: evidence for linear
    \end{itemize}
\end{frame}
...
\begin{frame}{Rewards}
    RewArt is not a necessity: advocacy for simple rewards (e.g. Walk-in-the-park).
...
\begin{frame}{Wrapping environments}
    ...
    \begin{itemize}
        ...
        \item (Training:) low-pass filter on wheel actuators
        \item Include last ten observation-action pairs in observation vector
        \item Change action to ground acceleration (with bounds)
        \item Rescale action between $-1$ and $1$
    \end{itemize}
\end{frame}
