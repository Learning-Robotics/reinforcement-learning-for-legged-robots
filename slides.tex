...
\begin{frame}{Parameterized policy}
    ...
    \]
    where $\mu_\theta$ and $\sigma_\theta$ are neural networks mapping states to means and standard deviations.

    (Common practice: $\sigma$ does not depend on $s$, put $\log \sigma \in \mathbb{R}^m$ rather than $\sigma \in \mathbb{R}^m_+$ in $\theta$.)
\end{frame}
